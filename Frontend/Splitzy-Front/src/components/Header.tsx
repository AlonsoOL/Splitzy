...
function Header() {
  ...
  return (
    <div className="w-full bg-transparent px-4 md:px-0">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row py-4 md:py-0">
        ...
        <div className="flex flex-row justify-center md:justify-end w-full md:w-1/2 gap-x-4 md:gap-x-8 items-center">
          {user && isAuthenticated ? (
            <div className="relative group inline-block">
              <div
                id=""
                className="hidden group-hover:flex bg-[#242424] flex-col block absolute top-full right-0 md:right-[-55px] z-10 rounded-[10px]"
              >
                <a href="/my-profile" className="h-full p-2 mt-2 hover:underline decoration-1">
                  Perfil
                </a>
                <Button
                  onClick={logout}
                  className="m-2 hover:bg-red-500! hover:text-red-50! hover:border-transparent! hover:transition!"
                >
                  Cerrar sesión
                </Button>
              </div>
          ...
          {user?.role === "Admin" && (
            <a href="/menu-admin">
              <svg
                className="fill-white hover:rotate-30 hover:fill-green-500 duration-100"
